...
let get_children (network : Network.t) (p : Person.t) =
  let contains_person (connection : Connection.T.t) =
    let person1, _person2 = connection in
    String.( = ) person1 p
  in
  Set.filter network ~f:contains_person
;;

let find_friend_group network person : Person.t list =
  let fr_group = [ person ] in
  let queue = [ person ] in
  let rec f network queue visited =
    match queue with
    | [] -> visited
    | head :: tail ->
      let n_visited =
        Set.fold
          (get_children network head)
          ~init:[]
          ~f:(fun acc ((_p1, p2) : Connection.T.t) ->
          if not (List.mem visited p2 ~equal:String.equal)
          then acc @ [ p2 ]
          else acc)
      in
      let n_queue = tail @ n_visited in
      f network n_queue (visited @ n_visited)
  in
  f network queue fr_group
;;
